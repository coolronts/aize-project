...
const App: React.FunctionComponent = () => {
  ...
  const addProductInfo = (_id:number,_quantity:number)=>{
    ...
    Get.getProduct(_id)
    .then(response=>{
      temp.push({info:response, quantity:_quantity})
    })
    .then(setProductsInfo((productsInfo.concat(temp))))
  }
  ...
  const addCart = async (_cart:ProductCartType[])=>{
    setProducts(()=>(_cart))
    await _cart.map(product =>{ 
      return addProductInfo(product.id, product.quantity)})
