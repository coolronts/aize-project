...
import {IProduct, ProductCartType} from './interfaces'
...
const App: React.FunctionComponent = () => {
  ...
  const addProductInfo = (id:number)=>{
    updateIsLoading(true)
    Get.getProduct(id)
    .then(response=>{
      setProductsInfo(productsInfo.concat(response))
    })
  }
  ...
  const addCart = (_cart:ProductCartType[])=>{
    setProducts(()=>(_cart))
    _cart.map(product =>{ return addProductInfo(product.id)})
  }
