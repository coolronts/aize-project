      ...
      <div className={styles.container}>
        <div className={styles.card}>
          ...
          {role ==='ADMIN'?
            ...
            {(edit === ''||'name') && <h4 className={styles.name}>  <FaEdit onClick={()=>{
              commonContext.updateIsEdit('name')
            }
              } className={styles.icon}/> {product.name}</h4>}
            {(edit === ''||'description') &&  <h2 className={styles.description}><FaEdit className={styles.icon}/>{product.description}</h2>}
