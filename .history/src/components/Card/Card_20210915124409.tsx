        ...
        <div className={styles.card}>
          ...
          {role ==='ADMIN'&&
            <div className={styles.adminControl}>
              ...
              <FaEdit onClick={()=>setIsEdit(true)} className={styles.icon}/>
            </div>
          }
          {isEdit ? (
          <div className={styles.editForm}>
            <input className={styles.name} value={product.name}/>
            <h2 className={styles.description}>{product.description}</h2>
            <h3 className={styles.price}>{product.price} <span>NOK</span></h3>
            <div className={styles.buttonContainer}>
              <button type="button" onClick={() => {userContext.addProduct(product.id,1)}}>Save</button>
            </div>
          </div>
          ):(
            <>
