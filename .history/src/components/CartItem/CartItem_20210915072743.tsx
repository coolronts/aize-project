...
interface IProp{
  id:number,qty:number,total:number, updateTotal:(arg:number) =>void
}
const CartItem: React.FunctionComponent<IProp> = ({id,qty,total,updateTotal}) => {
  ...
  useEffect(()=>{
    ...
    .then(response => {
      ...
      updateTotal(total+response.price*qty)
    })
  },[id])
  return (
    <>
