...
const EditModal: React.FunctionComponent = () => {  
    ...
    const save = ()=>{
      console.log(typeof(newValue))
        ...
        if(edit==='price' && typeof(newValue)==='number'){
          console.log(edit)
    ...
    <section className={styles.container}>
      <div className={styles.modal}>
        ...
        <h2>Edit the {edit} </h2>
        { (edit===('name'||'description')) && <input type="text" onChange={(e)=>setNewValue(e.target.value)}  value={newValue}  />}
        { (edit===('price'||'discount')) && <input type="number" step="any" onChange={(e)=>setNewValue(parseInt(e.target.value))}  value={newValue}  />}
